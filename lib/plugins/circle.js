(function ($, window){
    ...
    DefaultObj.prototype = {
        init : function(){
            ...
            _canvas.attr({
             ...
             if (this.setting.countDown) {
                ...
                __dountDown.attr({
                    id : 'counter_text',
                    width : this.setting.diameter,
                    height : this.setting.diameter
                });
                self.el.append(__dountDown);
            }

            self._drawCircle(_canvas, _diameter, _color, _lineWidth, this.setting.num);
         ...
         * is circle diameter
         */
        _drawCircle : function ( x, diameter, _color, _lineWidth, num){
            var circ = Math.PI * 2;
            var quart = Math.PI / 2;
            var imd = null;
            ...
            loadCanvas(1, x);
            if (this.setting.countDown) {
                this.__drawCountDown(num);
            }
        },

        __drawCountDown : function (num) {
            var interval;
            var _text = $('#counter_text');
            _text.html(num);
            num--;
